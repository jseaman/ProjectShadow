...
local CreateParticleType = function (name, Properties)
	...
	V.ParticleTypes[name].alphaMax              = 1.0
	...
	V.ParticleTypes[name].drawArea              = {0,0,V.gScrW,V.gScrH}
		...
		if V.Particles[i]  ~= nil then
			...
			if P.isPhysicsParticle ~= true then
				...
				if now < P.scaleOutTime or PType.so == 0 then	
					...
					if (PType.scaleLoop ~= true and P.scale < 0.01) or (PType.scaleLoop == true and P.scale < PType.scaleStart) then 
						-- LOOP SCALE?
						if PType.scaleLoop == true then
							P.scale        = PType.scaleStart
							P.scaleOutTime = now + PType.scaleOutDelay
						-- REMOVE WHEN INVISIBLE
						else P.killTime = now end
					end 
				end
			...
			if now < P.fadeOutTime or PType.fo == 0 then	
				...
				if (PType.fadeLoop ~= true and P.alfa < 0.01) or (PType.fadeLoop == true and P.alfa < PType.alphaStart) then 
					-- LOOP LPHA?
					if PType.fadeLoop == true then
						P.alfa        = PType.alphaStart
						P.fadeOutTime = now + PType.fadeOutDelay
					-- REMOVE WHEN INVISIBLE
					else P.killTime = now end
				end 
			end
			if P.alfa > PType.alphaMax then P.alfa = PType.alphaMax end
			P.alpha = (P.emitterAlpha * P.alfa > .01) and P.emitterAlpha * P.alfa or 0
			...
			if PType.killOutsideScreen == true or PType.bounceX == true or PType.bounceY == true then
				...
				if x < PType.drawArea[1] or x > PType.drawArea[3] then
				...
				elseif y < PType.drawArea[2] or y > PType.drawArea[4] then
