...
function scene:refreshState()
	if game.events.isTriggered("boys_locker_room.shade_baited") and not game.events.isTriggered("boys_locker_room.shade_baited_done") then
		if game.events.isTriggered("boys_locker_room.bait_shade_appears") then
		...
		if game.events.isTriggered("boys_locker_room.shade_baited") then
			if not game.events.isTriggered("boys_locker_room.bait_shade_appears") then
				self:newTimer(2000, function()
					...
					self:newTransition(self.shadeEnter, {alpha = 0.6, xScale = 1, yScale = 1, time = 1500, transition = easing.outSine, onComplete=function()
						self:newTimer(2000, function()
							scene:triggerEvent("boys_locker_room.bait_shade_appears")
						end)
					end})
				end)
			elseif not game.events.isTriggered("boys_locker_room.bait_shade_moves") then
				self:newTransition(self.shadeEnter, {x = device.x(160), time = 5000, onComplete=function()
					self:newTimer(1000, function()	
						scene:triggerEvent("boys_locker_room.bait_shade_moves")
					end)
				end})
			elseif not game.events.isTriggered("jason_locker_destroyed") then				
				...
				self:playSound("smash")
				system.vibrate()
								
				self:newTimer(1200, function()	
					self.robotTimer = game.ui.cancelTimer(self.robotTimer)
					self:playSound("robot_off")
				end)
				self:newTimer(1800, function()	
					scene:triggerEvent("jason_locker_destroyed")
				end)
			elseif not game.events.isTriggered("boys_locker_room.bait_shade_moves_back") then
				self:newTransition(self.shadeEnter, {x = device.x(415), time = 5000, onComplete=function()
					scene:triggerEvent("boys_locker_room.bait_shade_moves_back")	
				end})
			elseif not game.events.isTriggered("boys_locker_room.shade_baited_done") then
				self:newTransition(self.shadeEnter, {alpha = 0, xScale = 0.5, yScale = 0.5, time = 1500, transition = easing.outSine, onComplete=function()
					self:newTimer(2000, function()
						scene:stopSmoke()
						scene:triggerEvent("boys_locker_room.shade_baited_done")
						game.achievements.unlock("shade_bait")
						game.stage.playGameMusic("scene.game.gym.locker_lou_baited")
					end)
				end})
			end
		end
...
function scene:onCreate(event)
	...
	self:addToScene(game.ui.newSceneObjectAndDo("assets/images/game/gym/boys_locker_room/inside/locker_open_door.png", device.x(129), device.y(0), 272, 360, onExitTouch))
end
...
function scene:onShow(event)
	if event.phase == "will" then
		...
		self:loadSound("smash", "assets/sounds/game/gym/locker_smash.mp3")
		self:loadSound("robot_noise", "assets/sounds/game/gym/robot_noise.mp3")
		self:loadSound("robot_off", "assets/sounds/game/gym/robot_off.mp3")
		self:loadSound("shade", "assets/sounds/game/gym/shade_breath.mp3")
		self.smoke = nil
	elseif event.phase == "did" then
		game.hud.show()
		
		if not game.events.isTriggered("jason_locker_destroyed") then
			self.robotTimer = self:newTimer(800, function()
